...
class Node:
    def __init__(self, weight: int, smallest_weight: bool):
        self.weight = weight
        self.smallest_weight = smallest_weight # flag for contains smallest weight symbol

    def __add__(self, other):
        global max_len
        if self.smallest_weight or other.smallest_weight:
            max_len += 1
...
def huffman_code_max_len()->int:
    global symbol_num
    global symbol_list
    symbol_list.sort()
    raw_queue = []
    for i in range(0, symbol_num):
        raw_queue.append(Node(weight=symbol_list[i], smallest_weight=False))
    raw_queue[0].smallest_weight = True
    meta_queue = []
    merge_count = 0
